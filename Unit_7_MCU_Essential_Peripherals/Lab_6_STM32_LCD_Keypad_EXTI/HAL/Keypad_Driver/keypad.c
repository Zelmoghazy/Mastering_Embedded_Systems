...
unsigned char key_press(void)
{
    ...
    gpio_write_pin(KEYPAD_PORT, ALL_ROWS, 0);        // enable all rows
    col = gpio_read_port(KEYPAD_PORT) & ALL_COLS;    // read all columns
...
// debouncing algorithm on top of get_key
...
unsigned char keypad_getkey(void)
{
    ...
    while (1)
    {
        ...
        //----------------------------------------------
        row = 0;
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_0, 0);  // enable row 0 only
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_1, 1);  
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_2, 1);  
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_3, 1);  
        ...
        row = 1;
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_0, 1);  
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_1, 0);  // enable row 1 only
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_2, 1);  
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_3, 1);  
        ...
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_0, 1);  
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_1, 1);  
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_2, 0);  // enable row 2 only
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_3, 1);  
        _delay_us(2);               
        col = gpio_read_port(KEYPAD_PORT) & ALL_COLS; // read all columns
        if (col != ALL_COLS)
        ...
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_1, 1);  
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_2, 1);  
        gpio_write_pin(KEYPAD_PORT,ROW_PIN_3, 0);   // enable row 3 only
        _delay_us(2);              
        col = gpio_read_port(KEYPAD_PORT) & ALL_COLS; // read all columns
        if (col != ALL_COLS)
            break;
