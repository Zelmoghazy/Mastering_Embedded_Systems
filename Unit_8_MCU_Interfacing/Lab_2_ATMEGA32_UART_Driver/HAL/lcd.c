...
{
	gpio_write(lcd->rs_port, lcd->rs_pin, LCD_COMMAND_REG);		// Write to command register
...
{
	gpio_write(lcd->rs_port, lcd->rs_pin, LCD_DATA_REG);			// Write to data register
...
uint8_t lcd_busy_flag(lcd_handle_t * lcd)
{
	...
	gpio_write(lcd->rw_port, lcd->rw_pin, LCD_READ);
	gpio_write(lcd->rs_port, lcd->rs_pin, 0);		//Busy flag: address counter (for read)

    // Read busy flag (DB7) and address counter (DB0 to DB6)
    uint8_t state = gpio_read(GPIO_C, GPIO_PIN_7);
	...
	gpio_write(lcd->rw_port, lcd->rw_pin, LCD_WRITE);
